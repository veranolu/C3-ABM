# -*- coding: utf-8 -*-
# c3_abm_panel_v2.py -- A方案: 仿真新口径重算 (REWB 面板化) v2 内存优化
# CHANGELOG:
#   v1 2026-09-12 复制自 c3_abm.py, 加窗口面板 -> vefaas 2GB OOM 被杀
#   v2 2026-09-12 内存优化: theta 初始化后即删; x*n 用 float32 原地乘(out=复用x)
#     避免 float64 204MB 临时; rng 调用序列与官方版完全一致(逐窗结算不改流);
#     lam 由 f64 降 f32 是唯一实现差异 -> headline 允许 +-0.001 级实现差
...
# 口径: 10窗 x 10期; maxshare=窗内行为份额max, entropy=窗内Shannon熵, n_click=窗内选择总数
# 估计: Mundlak OLS entropy ~ mean_maxshare + dev, cluster uid; CDI_v2_sim = bB - bW
...
def run_model_panel(N=25467, T=100, K=1000, delta=0.003, lam_g=0.05,
    ...
    x = np.empty((N, K), dtype=np.float32)       # v2: 常驻复用
    ...
    for t in range(T):
        ...
        np.multiply(x, n32[:, None], out=x)      # v2: f32 原地, 无 f64 临时
        c = rng.poisson(x).astype(np.float32)    # int64 临时一次性
...
if __name__ == '__main__':
    ...
    for cond in conds:
        ...
        out[cond] = r
        with open('/Coze/Drive/扣子/所有对话/主对话/C3_仿真本体_20260912/simA_results.json', 'w') as f:
